...
//		parametro.borderWidth = 1;
    ...
    public void setColorTexto(Color colorLetra, Color colorBorde) {
        this.setColor(colorLetra);
        this.setBorderColor(colorBorde);
    }

    public boolean mouseDentroDeTexto(Entrada entrada) {
        return (entrada.getMouseX() > posicion.x && entrada.getMouseX() < (posicion.x + getWidth()) && (entrada.getMouseY() > posicion.y - getHeight() && entrada.getMouseY() < (posicion.y + getHeight()) - getHeight()));
    }
}
